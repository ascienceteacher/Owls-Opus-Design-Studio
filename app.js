...
const APP_VERSION = "2.1.1-google-save";
...
const steps = ["home","projectPath","drawProblem","drawPower","drawInspiration","cards","problem","power","inspiration","mashup","whatif","originality","science","studio","pitch","summary","inventorWall"];

function blankState(){
  return {step:0, student:{name:"", period:"", teacher:""}, problem:null, power:null, inspiration:null, answers:{}, sidekick:{}, notebook:{}, badges:[]};
}

let currentProjectKey = sessionStorage.getItem("owlProjectKey") || "";
let state = JSON.parse(sessionStorage.getItem(SESSION_KEY) || "null") || blankState();
...
function save(){
  ...
  scheduleGoogleAutosave();
}
function scheduleGoogleAutosave(){
  if(!currentProjectKey || !GOOGLE_SAVE_URL) return;
  clearTimeout(googleSaveTimer);
  googleSaveTimer = setTimeout(()=>silentSaveToGoogleSheet(), 1200);
}
async function silentSaveToGoogleSheet(){
  if(!currentProjectKey) return;
  try{
    await googleRequest({
      action:"save",
      projectKey:currentProjectKey,
      inventionTitle:state.answers?.pitch1 || state.answers?.mashupIdeas || "Untitled Invention",
      projectData:state
    });
  }catch(err){
    console.warn("Google autosave failed", err);
  }
}
...
const stepLabels = {
  home:["Start","Welcome"],
  projectPath:["Start","Choose Your Path"],
...
function canVisitStep(index){
  ...
  if(step === "projectPath") return true;
  if(!state.student?.name || !state.student?.period || !state.student?.teacher) return step === "home" || step === "projectPath";
...
function stepIsDone(step){
  ...
  if(step === "projectPath") return !!(state.student?.name && state.student?.period && state.student?.teacher);
...
function sparkStageForStep(stepName){
  if(["home","projectPath","drawProblem","problem"].includes(stepName)) return "S";
...
function missionInfoForStep(stepName){
  const map = {
    projectPath:["🦉","Start","Choose Your Inventor Path","Start fresh or return to a project you already saved."],
...
function renderSparkTrail(){
  ...
  return `<div class="spark-trail no-print">
    ...
    ${items.map(([letter,title,desc])=>`<div class="spark-step stage-${letter.toLowerCase()} ${letter===active?'active':''}"><b>${letter}</b><span>${title}</span><small>${desc}</small></div>`).join('<div class="spark-arrow">→</div>')}
  </div>`;
}
  ...
  app.innerHTML = `<div class="app theme-refresh ${isHome?'home-app':''}">
    <div class="topbar no-print">
      ...
      <div class="nav">
        ...
        <button class="nav-tile ${steps[state.step]==='projectPath'?'active':''}" onclick="jumpToStep(${steps.indexOf('projectPath')})"><span>▣</span>Start / Load</button>
        <button class="nav-tile save-link ${currentProjectKey?'':'hidden'}" onclick="saveCurrentProject()"><span>💾</span>Save Work</button>
...
function render(){
  ...
  if(step==="projectPath") return renderProjectPath();
...
function startFromHome(){
  state.step = steps.indexOf("projectPath");
  save();
  render();
}
...
function periodOptions(selected=""){
  return PERIODS.map(p=>`<option value="${esc(p)}" ${p===selected?'selected':''}>${esc(p)}</option>`).join("");
}

function renderProjectPath(){
  shell(`<section class="path-page">
    <div class="path-hero">
      <div class="stage-kicker">Start • Owl's Opus 2.1</div>
      <h2>Choose Your Inventor Path</h2>
      <p>Start with a blank project or return to work you saved using your name, period, and 4-digit PIN.</p>
    </div>
    <div class="path-grid">
      <div class="path-card new-project-card">
        <div class="path-icon">🚀</div>
        <h3>Start a New Project</h3>
        <p>Begin a brand-new invention. Your screen will be blank and ready for a new S.P.A.R.K. mission.</p>
        <div class="path-form">
          <label>First Name<input id="newFirstName" autocomplete="off" placeholder="First name"></label>
          <label>Last Initial<input id="newLastInitial" autocomplete="off" maxlength="1" placeholder="Last initial"></label>
          <label>Period<select id="newPeriod">${periodOptions()}</select></label>
          <label>4-digit PIN<input id="newPin" autocomplete="off" maxlength="4" inputmode="numeric" placeholder="Create PIN"></label>
        </div>
        <button class="mock-start path-button" onclick="startNewProjectFromPath()">Start New Project <span>→</span></button>
      </div>
      <div class="path-card continue-project-card">
        <div class="path-icon">📂</div>
        <h3>Continue Saved Project</h3>
        <p>Return to work you already started. Your project stays private unless you submit it to the Showcase.</p>
        <div class="path-form">
          <label>First Name<input id="loadFirstName" autocomplete="off" placeholder="First name"></label>
          <label>Last Initial<input id="loadLastInitial" autocomplete="off" maxlength="1" placeholder="Last initial"></label>
          <label>Period<select id="loadPeriod">${periodOptions()}</select></label>
          <label>4-digit PIN<input id="loadPin" autocomplete="off" maxlength="4" inputmode="numeric" placeholder="Enter PIN"></label>
        </div>
        <button class="mock-start path-button secondary-path" onclick="loadProjectFromPath()">Load My Project <span>→</span></button>
      </div>
    </div>
    <div class="path-note"><b>Teacher note:</b> Student work saves privately to the Google Sheet save system. Showcase items only appear after students submit them.</div>
  </section>`);
}

function getPathLogin(prefix){
  const firstName = document.getElementById(prefix+"FirstName")?.value.trim();
  const lastInitial = document.getElementById(prefix+"LastInitial")?.value.trim().slice(0,1).toUpperCase();
  ...
  if(!firstName || !lastInitial || !period || !pin){
    alert("Please enter first name, last initial, period, and PIN.");
    return null;
  }
  if(!/^\d{4}$/.test(pin)){
    alert("Please use a 4-digit PIN.");
    return null;
  }
  return {firstName,lastInitial,period,pin};
}

async function googleRequest(payload){
  const response = await fetch(GOOGLE_SAVE_URL, {method:"POST", body: JSON.stringify(payload)});
  return await response.json();
}

async function startNewProjectFromPath(){
  const login = getPathLogin("new");
  if(!login) return;
  const fresh = blankState();
  fresh.student = {name:`${login.firstName} ${login.lastInitial}.`, period:login.period, teacher:"Stasny"};
  try{
    const result = await googleRequest({action:"login", ...login, projectData:fresh});
    if(result.success){
      currentProjectKey = result.projectKey;
      sessionStorage.setItem("owlProjectKey", currentProjectKey);
    }
  }catch(err){
    alert("This project will start on this device. Google saving did not connect yet.");
  }
  state = fresh;
...
async function loadProjectFromPath(){
  const login = getPathLogin("load");
  if(!login) return;
  try{
    const result = await googleRequest({action:"load", ...login});
    if(!result.success){ alert(result.message || "Project not found."); return; }
    currentProjectKey = result.projectKey;
    sessionStorage.setItem("owlProjectKey", currentProjectKey);
    state = result.projectData || blankState();
    state.student = state.student || {name:`${login.firstName} ${login.lastInitial}.`, period:login.period, teacher:"Stasny"};
    state.step = state.step && state.step > 0 ? state.step : steps.indexOf("drawProblem");
    save();
    alert("Your saved project loaded.");
    render();
  }catch(err){
    alert("The saved project could not be loaded. Check the Google Sheet save system and try again.");
  }
}

async function saveCurrentProject(){
  if(!currentProjectKey){ alert("Use Start / Load first so Owl's Opus knows which project to save."); return; }
  saveInputs();
  try{
    ...
    alert(result.success ? "Project saved to Google Sheets." : (result.message || "Project could not be saved."));
  }catch(err){
    alert("Google saving is not connected right now. Your work is still available in this browser session.");
  }
}

function renderDrawProblem(){
  shell(`<section class="phase2-stage deck-graphic-removed stage-problem">
    <div class="stage-copy clean-deck-intro">
...
function renderDrawPower(){
  shell(`<section class="phase2-stage deck-graphic-removed stage-power">
    <div class="stage-copy clean-deck-intro">
...
function renderDrawInspiration(){
  shell(`<section class="phase2-stage deck-graphic-removed stage-inspiration">
    <div class="stage-copy clean-deck-intro">
...
async function submitResult(){
  ...
  if(currentProjectKey && GOOGLE_SAVE_URL){
    try{
      await googleRequest({
        action:"submitShowcase",
        projectKey:currentProjectKey,
        inventionTitle:state.answers?.pitch1 || state.answers?.mashupIdeas || "Untitled Invention",
        problemSolved:getCardTitle(state.problem),
        favoriteFeature:getCardTitle(state.power),
        scienceConnection:state.answers?.science_text || "",
        pitch:state.answers?.pitch1 || state.answers?.mashupIdeas || ""
      });
    }catch(err){
      console.warn("Google Showcase submit failed", err);
    }
  }
...
async function getGoogleTeacherResults(){
  if(!GOOGLE_SAVE_URL) return null;
  try{
    const result = await googleRequest({action:"getProjects"});
    if(!result.success) return null;
    return {source:"Google Sheet", results:result.items || []};
  }catch(err){
    console.warn("Google Sheet teacher results failed", err);
    return null;
  }
}
...
async function getTeacherResults(teacher){
  ...
  const googleResults = await getGoogleTeacherResults();
...
async function renderTeacherHub(teacher){
  ...
  app.innerHTML = `<div class="app theme-refresh">
    ...
    <section class="sprint4-page teacher-page">
      ...
      <div class="firebase-status sprint4-status">Data source: ${esc(source)} ${source==="Firestore" || source==="Google Sheet" ? "✅ Shared classroom dashboard connected." : "⚠️ Local browser storage only."}</div>
